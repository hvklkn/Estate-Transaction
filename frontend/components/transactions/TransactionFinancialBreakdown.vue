...
<template>
  <section class="rounded-xl border border-slate-200 bg-slate-50/80 p-4">
    <div class="flex items-start justify-between gap-3">
      <h5 class="text-sm font-semibold text-slate-800">{{ t('transactions.financial.title') }}</h5>
      <p v-if="props.totalServiceFee" class="text-xs text-slate-500">
        {{ t('transactions.financial.total') }}: {{ formatCurrency(props.totalServiceFee) }}
      </p>
    </div>

    <dl class="mt-3 grid gap-3 md:grid-cols-2">
      <div class="rounded-lg border border-brand-200 bg-white px-3 py-3">
        ...
        <p v-if="props.totalServiceFee" class="mt-1 text-xs text-slate-500">
    ...
    <div v-if="props.financialBreakdown.agents.length > 0" class="mt-3 space-y-2.5">
      ...
      >
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <p class="text-base font-semibold text-slate-900">{{ formatCurrency(agent.amount) }}</p>
        </div>
        <p class="mt-1.5 text-xs leading-5 text-slate-500">{{ agent.explanation }}</p>
      </article>
    </div>

    <p v-else class="mt-3 rounded-lg border border-dashed border-slate-300 bg-white px-3 py-2 text-xs text-slate-500">
