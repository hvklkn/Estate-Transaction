...
const headerThemeClasses = computed(() => 'border-transparent bg-gradient-to-r from-sky-600 via-blue-600 to-indigo-600 shadow-[0_6px_20px_rgba(37,99,235,0.35)]');

const brandButtonClasses = computed(() => 'border-white/90 bg-white text-blue-700 shadow-[0_6px_14px_rgba(255,255,255,0.35)] hover:border-white hover:bg-blue-50 hover:text-blue-800');

const panelClasses = computed(() => 'border-white/50 bg-blue-900/20 shadow-sm backdrop-blur hover:border-white/80 hover:bg-blue-900/30');
...
<template>
  <div class="min-h-screen bg-brand-50/60 text-slate-800 dark:bg-slate-950 dark:text-slate-100">
    <header class="sticky top-0 z-10 border-b backdrop-blur" :class="headerThemeClasses">
      <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6">
        <div class="flex items-center justify-between gap-3">
          <div class="flex min-w-0 items-center gap-2 sm:gap-3">
            <NuxtLink to="/" class="inline-flex items-center rounded-xl border px-3 py-2 text-sm font-semibold tracking-tight shadow-sm transition-all hover:shadow" :class="brandButtonClasses">
              <span class="sm:hidden">ET</span>
              <span class="hidden sm:inline">Estate Transaction</span>
            </NuxtLink>

            <nav class="hidden items-center gap-2 md:flex">
              <NuxtLink v-for="navigationItem in navigationItems" :key="navigationItem.to" :to="navigationItem.to" class="rounded-lg border px-3 py-2 text-xs font-semibold uppercase tracking-[0.08em] transition-colors" :class="isRouteActive(navigationItem.to) ? 'border-white bg-white text-blue-700 shadow-sm' : 'border-white/45 bg-blue-900/20 text-blue-100 hover:border-white/80 hover:bg-blue-900/35 hover:text-white'">
          ...
          <div class="hidden min-w-0 items-center gap-2 text-sm sm:gap-3 md:flex">
            <button type="button" class="relative inline-flex h-8 w-16 items-center rounded-full border border-white/50 bg-blue-900/25 p-1 transition-colors hover:border-white/80 hover:bg-blue-900/35" :aria-label="colorMode === 'dark' ? t('layout.theme.switchToLight') : t('layout.theme.switchToDark')" :aria-pressed="colorMode === 'dark'" @click="toggleColorMode">
              <span class="flex w-full items-center justify-between px-1 text-[11px] leading-none text-blue-100">
                <span>☀</span>
                <span>🌙</span>
              </span>
              <span class="absolute top-1 h-6 w-6 rounded-full bg-white text-center text-sm leading-6 text-blue-700 shadow transition-all" :class="colorMode === 'dark' ? 'left-9' : 'left-1'">
                {{ colorMode === 'dark' ? '🌙' : '☀' }}
              </span>
            </button>

            <div v-if="currentUserName" class="inline-flex max-w-[300px] items-center gap-3 rounded-lg border px-2 py-1.5 transition-colors lg:max-w-[380px] lg:px-3" :class="panelClasses">
              <div class="min-w-0">
                <p class="truncate text-xs font-semibold text-white">
                  {{ currentOrganizationName }}
                </p>
                <p class="truncate text-[11px] text-blue-100">{{ currentUserName }} · {{ currentUserRoleLabel }}</p>
              </div>
              <button type="button" class="shrink-0 text-xs font-medium transition-colors" :class="'text-blue-100 hover:text-white'" @click="handleLogout">
          ...
          <div class="flex items-center gap-2 md:hidden">
            ...
            <button type="button" class="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/50 bg-blue-900/25 text-white transition-colors hover:border-white hover:bg-blue-900/35" :aria-label="isMobileMenuOpen ? t('layout.menu.close') : t('layout.menu.open')" :aria-expanded="isMobileMenuOpen" @click="toggleMobileMenu">
        ...
        <div v-if="isMobileMenuOpen" class="mt-3 rounded-xl border border-white/40 bg-blue-900/40 p-3 md:hidden">
          <nav class="grid gap-2">
            <NuxtLink v-for="navigationItem in navigationItems" :key="navigationItem.to" :to="navigationItem.to" class="rounded-lg border px-3 py-2 text-sm font-medium transition-colors" :class="isRouteActive(navigationItem.to) ? 'border-white bg-white text-blue-700' : 'border-white/40 bg-blue-900/20 text-blue-100 hover:border-white/80 hover:bg-blue-900/30'">
              {{ navigationItem.label }}
            </NuxtLink>
          </nav>

          <div class="mt-3 space-y-2 border-t border-white/25 pt-3">
            <div v-if="currentUserName" class="flex items-center justify-between gap-3 rounded-lg border border-white/35 bg-blue-900/25 px-3 py-2">
              <div class="min-w-0">
                <p class="truncate text-xs font-semibold text-white">
                  {{ currentOrganizationName }}
                </p>
              ...
              <button type="button" class="shrink-0 text-xs font-medium text-blue-100 transition-colors hover:text-white" @click="handleLogout">
                {{ t('auth.actions.logout') }}
              </button>
            </div>
          </div>
      ...
      <div class="mx-auto max-w-7xl py-8">
        <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          <section class="space-y-2">
            ...
            <p class="text-xs leading-5 text-slate-500 dark:text-slate-400">Real estate transaction lifecycle, stage traceability, and commission visibility in one operational dashboard.</p>
          </section>
          ...
          <section class="space-y-2">
            ...
            <div class="flex flex-col gap-1.5 text-sm">
              ...
              <NuxtLink to="/clients" class="transition-colors hover:text-brand-700 dark:hover:text-brand-300">Clients</NuxtLink>
              <NuxtLink to="/properties" class="transition-colors hover:text-brand-700 dark:hover:text-brand-300">Properties</NuxtLink>
              <NuxtLink to="/tasks" class="transition-colors hover:text-brand-700 dark:hover:text-brand-300">Tasks</NuxtLink>
              <NuxtLink to="/balance" class="transition-colors hover:text-brand-700 dark:hover:text-brand-300">Balance</NuxtLink>
              <NuxtLink v-if="authStore.canManageTeam" to="/team" class="transition-colors hover:text-brand-700 dark:hover:text-brand-300">Team</NuxtLink>
        ...
        <div class="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4 text-xs text-slate-500 dark:border-slate-800 dark:text-slate-400">
          ...
          <span class="rounded-full border border-slate-300 bg-slate-100 px-2.5 py-1 font-semibold text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300"> ENV: {{ appEnv }} </span>
        </div>
      </div>
    </footer>
  </div>
